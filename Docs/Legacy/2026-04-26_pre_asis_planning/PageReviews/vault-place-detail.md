# Page Review: Place Detail

## Summary
- Route: `/vault/places/[placeId]`
- Domain: Vault
- Primary user job: 장소 상세와 연결된 사람/날짜/source를 확인한다.
- Overall score: 23/33
...
- Reviewed code: `apps/web/src/app/(app)/vault/places/[placeId]/page.tsx`, `apps/web/src/components/shared/context/entity-context-shell.tsx`
...
## Analysis
- Route / components: server finds place from vault snapshot and renders EntityContextShell with people lens.
- Server data / D1: place row from D1; ContextBundle can collect daily/person/source relations.
- Contextual Connectivity: detail has context rail and source trace through shell.
- Mobile UX: straightforward layout, context rail becomes accordion.
...
- Main slot is read-only; users cannot add visit/event context from the place detail itself.
...
### P2/P3 Actions
- [ ] Add “record visit” or attach person/daily log CTA in main slot.
- [ ] Remove duplicate seed call in detail route.
- [ ] Verify 375px context/source panels.
