...
export default async function ThisWeekPage() {
  ...
  const [weeklyRhythm, tasks, needsContactReadModel] = await Promise.all([getLifeOpsWeeklyRhythm(dates), getActionHubTasks(), getPRMNeedsContact()]);
  const activeTasks = tasks.filter((task) => task.status !== "done").slice(0, 8);
  const needsContact = needsContactReadModel.slice(0, 6);

  return (
    <section className="space-y-4">
      <GlassCard className="p-6">
        <p className="text-xs uppercase tracking-[0.24em] text-primary">오늘 보기</p>
        <h1 className="mt-3 font-display text-4xl text-foreground">이번 주</h1>
        <p className="mt-3 max-w-2xl text-sm text-muted-foreground">최근 7일의 에너지, 습관, 작업, 연락 리듬을 주간 계획 화면으로 모았습니다.</p>
      </GlassCard>
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_380px]">
        <GlassCard className="p-5">
          <p className="text-xs uppercase tracking-[0.2em] text-primary">7일 리듬</p>
          <div className="mt-5 grid gap-3 md:grid-cols-7">
            {weeklyRhythm.map((log) => {
              const date = log.date;
              return (
                <Link className="rounded-lg border border-white/10 bg-white/5 p-3 hover:bg-white/8" href={`/life-ops/${date}`} key={date}>
                  ...
                  <p className="mt-1 text-[11px] uppercase tracking-[0.14em] text-muted-foreground">에너지</p>
                </Link>
              );
            })}
          </div>
        ...
        <div className="space-y-4">
          <GlassCard className="p-5">
            <p className="text-xs uppercase tracking-[0.2em] text-primary">몰입 대기열</p>
            <div className="mt-4 space-y-2">
              ...
              )) : <p className="rounded-lg border border-dashed border-white/10 bg-white/5 p-3 text-sm text-muted-foreground">이번 주에 표시할 활성 작업이 없습니다.</p>}
            </div>
          </GlassCard>
          <GlassCard className="p-5">
            <p className="text-xs uppercase tracking-[0.2em] text-primary">연락 필요</p>
            <div className="mt-4 space-y-2">
              {needsContact.length ? needsContact.map((person) => (
                <Link className="flex items-center justify-between rounded-lg border border-white/10 bg-white/5 p-3 text-sm hover:bg-white/8" href={`/prm/${person.id}`} key={person.id}>
