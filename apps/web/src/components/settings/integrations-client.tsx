...
const JOBS: Array<{ id: CronJob; label: string }> = [
  { id: "hit_them_up", label: "Hit-Them-Up" },
  ...
  return (
    <section className="grid gap-4 xl:grid-cols-2">
      ...
      <div className="glass rounded-[20px] p-5">
        ...
        <h2 className="mt-3 font-display text-4xl text-foreground">Cron 테스트</h2>
        <div className="mt-5 flex flex-wrap gap-2">
          {JOBS.map((job) => (
            <button
              className="rounded-2xl border border-white/10 px-3 py-2 text-sm text-foreground disabled:opacity-60"
              disabled={isPending}
              key={job.id}
              onClick={() => {
          ...
          <div className="mt-3 space-y-2">
            {initial.recentRuns.length ? (
              initial.recentRuns.map((run) => (
                <div className="rounded-2xl border border-white/10 bg-white/5 px-3 py-3 text-sm" key={run.id}>
