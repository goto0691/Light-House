...
export function SavedViewTabs({ activeViewKey, basePath, views }: SavedViewTabsProps) {
  return (
    <nav aria-label="Saved views" className="flex max-w-full min-w-0 gap-2 overflow-x-auto rounded-lg border border-white/10 bg-white/5 p-1">
      {views.map((view) => {
        ...
        return (
          <Link
            ...
            className={cn(
              "focus-ring shrink-0 whitespace-nowrap rounded-md px-3 py-2 text-xs font-medium uppercase tracking-[0.16em] transition",
