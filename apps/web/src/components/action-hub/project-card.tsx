  ...
  return (
    <Link className="glass block rounded-[28px] p-5 transition hover:shadow-[var(--shadow-glow),var(--shadow-lg)]" href={`/action-hub/${project.id}`} scroll={false}>
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            <Tag value={optionLabel(PROJECT_KIND_OPTIONS, project.kind, project.kind)} variant="neutral" />
          </div>
        </div>
      ...
      <div className="mt-5 flex items-center gap-2">
        <Tag value={optionLabel(PROJECT_STATUS_OPTIONS, project.status, project.status)} variant="status" />
        <span className="text-xs tracking-[0.08em] text-muted-foreground">{project.dueLabel}</span>
      </div>

      <div className="mt-5 flex items-center justify-between text-sm">
        <span className="text-muted-foreground">{project.recentActivity}</span>
        <span className="text-foreground">열기</span>
      </div>
    </Link>
  );
}
