  ...
  return (
    <GlassCard priority="secondary">
      <div className="flex items-center gap-2">
        ...
        <p className="text-xs uppercase tracking-[0.2em] text-primary">Link Composer</p>
      </div>
      ...
      <div className="mt-4 space-y-2">
        {filteredCandidates.length ? (
          filteredCandidates.map((zettel) => (
            <button
              className="focus-ring flex w-full items-start justify-between gap-3 rounded-md border border-white/10 bg-white/5 px-3 py-3 text-left transition hover:bg-white/8 disabled:cursor-not-allowed disabled:opacity-60"
              disabled={disabled}
              key={zettel.id}
            ...
            >
              <span className="min-w-0">
                ...
                <span className="mt-1 line-clamp-1 block text-xs text-muted-foreground">{zettel.category} · {zettel.type}</span>
              </span>
              <span className="shrink-0 rounded-full border border-primary/20 bg-primary/10 px-2 py-1 text-[10px] uppercase tracking-[0.14em] text-primary">Link</span>
            </button>
          ))
        ) : (
          <p className="rounded-md border border-dashed border-white/15 bg-white/5 p-3 text-sm text-muted-foreground">검색 조건에 맞는 메모가 없습니다.</p>
        )}
      </div>

      {suggested.length ? (
        <div className="mt-5 border-t border-white/10 pt-4">
          <p className="text-xs uppercase tracking-[0.16em] text-muted-foreground">Semantic suggestions</p>
          <div className="mt-3 space-y-2">
            {suggested.map((item) => (
              <button
                className="focus-ring block w-full rounded-md border border-white/10 bg-black/10 px-3 py-3 text-left transition hover:bg-white/8 disabled:cursor-not-allowed disabled:opacity-60"
                disabled={disabled}
                key={item.id}
